...
public class StoreAggregate extends AggregateRoot<StoreId> {
  ...
  @Getter(PACKAGE)
  private final Map<ProductId, Map<ProductVariationId, ProductVariationPieces>> products = new HashMap<>();
  ...
  public void addProductVariationPieces(
      ProductId product, ProductVariationId variation, ProductVariationPieces pieces
  ) {
    if (pieces.isNone()) {
      return;
    }
  ...
  private void handle(ProductVariationPiecesAdded event) {
    this.products.compute(event.product(), (product, variations) -> {
      if (isNull(variations)) {
        var newVariations = new HashMap<ProductVariationId, ProductVariationPieces>();
  ...
  static StoreAggregate restore(
      ...
      StoreId id,
      Address address,
      Map<ProductId, Map<ProductVariationId, ProductVariationPieces>> products
  ) {
