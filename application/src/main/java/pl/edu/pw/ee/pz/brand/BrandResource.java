...
public class BrandResource {
  ...
  public Uni<CreateBrandResponse> create(
    ...
    return readLogoInputStream(logoImage)
        ...
        .onItem().transformToUni(commandHandlerExecutor::execute)
        .onItem().transform(Unchecked.function(result -> {
          if (result instanceof BrandId brandId) {
            return new CreateBrandResponse(brandId.id().toString());
          } else {
            throw new RuntimeException("Result is not a BrandId instance: " + result.getClass().getName());
          }
        }));
  }
  ...
  public Uni<RestResponse<Void>> changeBrandCode(ChangeBrandCodeRequest request, String id) {
    var command = new ChangeBrandCodeCommand(
        new BrandId(UUID.fromString(id)), new BrandCode(request.getCode()));
